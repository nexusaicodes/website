...
export default function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-8 py-4 bg-black/30 backdrop-blur-md z-[9999]">
      {/* Logo and Brand */}
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <Link href="/">
          <span className="text-xl md:text-2xl font-bold tracking-tight ml-1 md:ml-2">
            Nexus AI
          </span>
        </Link>
      </div>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="w-full py-6">
                {navItems.map((item, idx) => (
                  ...
                  >
                    <button
                      className="w-full text-left px-6 py-4 text-lg font-semibold text-white flex items-center justify-between hover:bg-white/10 transition-colors"
                      ...
                      <div className="bg-white/5">
                        {item.dropdown.map((sub) => (
                          <a
                            key={sub.title}
                            href={sub.href}
                            className="block px-6 py-3 text-white/80 hover:text-white hover:bg-white/10 transition-colors text-base"
                            onClick={() => {
                              console.log(
